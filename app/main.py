...
async def get_user_byname(
    username: str, status_code=status.HTTP_200_OK, response_model=UserOut
):
    user_db_dict = await db.users.find_one({"username": username})
    try:
        user_out = UserOut(**user_db_dict)
...
@app.get(
    "/optionsnurse", status_code=status.HTTP_200_OK, response_model=List[UserSelect]
)
...
def get_mad_report_filename(examination):
    ...
    file_name = (
        file_name.strip()
        + "_"
        + examination.aesqulap_pid.strip()
        + "_"
        + examination.last_name.strip()
        + "_"
        + examination.first_name.strip()
        + "_"
        + examination.date_of_birth.strftime("%d-%m-%Y")
        + ".pdf"
    )
    return file_name
...
async def generate_examination_report(request: Request, id: str):
    ...
    items = [
        {
            "time_stamp": utc_tz.localize(item["time_stamp"]).astimezone(local_tz),
            "text": item["text"],
        }
    ...
    return templates.TemplateResponse(
        "index.html", {"request": request, "examination": examination}
    )
...
async def generate_pdf_report(examination):
    ...
    if browser_path == None:  # Local dev environment on mac
        ...
        browser = await launch(
            executablePath=browser_path, headless=True, args=["--no-sandbox"]
        )
...
async def update_patient(
    id: str, patient: Patient, response: Response, request: Request
):
    patient_doc = patient.dict()
    _ = patient_doc.pop("id", None)
...
async def create_examination(
    examination: Examination, response: Response, request: Request
):
...
async def update_examination(
    id: str, examination: Examination, response: Response, request: Request
):
    examination_doc = examination.dict()
    _ = examination_doc.pop("id", None)
    # examination_doc['examination_date'] = dt.datetime.combine(examination_doc['examination_date'], dt.time.min)
...
async def start_examination(id: str, response: Response, request: Request):
    ...
    if examination.state == "planned":
        ...
        if last_examination:
            ...
            examination.premedication.patient_height = (
                last_examination.premedication.patient_height
            )
            examination.premedication.patient_weight = (
                last_examination.premedication.patient_weight
            )
            examination.premedication.patient_bmi = (
                last_examination.premedication.patient_bmi
            )
            examination.premedication.has_allergies = (
                last_examination.premedication.has_allergies
            )
            examination.premedication.allergies = (
                last_examination.premedication.allergies
            )
            examination.premedication.asa_class = (
                last_examination.premedication.asa_class
            )
            examination.premedication.medication = (
                last_examination.premedication.medication
            )
            examination.premedication.cardio_diseases = (
                last_examination.premedication.cardio_diseases
            )
            examination.premedication.respiratory_diseases = (
                last_examination.premedication.respiratory_diseases
            )
            examination.premedication.visceral_diseases = (
                last_examination.premedication.visceral_diseases
            )
            examination.premedication.neuro_diseases = (
                last_examination.premedication.neuro_diseases
            )
            ...
            message[
                "text"
            ] = f"Prämedikationsdaten kopiert von der Untersuchung vom {date_loc.strftime('%d.%m.%Y %H:%M')} für {examination.first_name} {examination.last_name}."
        else:
        ...
        await db.examinations.replace_one(
            {"_id": ObjectId(id)}, updated_examination_doc
        )
...
async def read_examinations_with_filter(
    state: str = None, planned_date: dt.datetime = None, mandant: str = None
):
    query = {}
    if state:
...
async def read_examination(id: str, status_code=status.HTTP_200_OK):
    ...
    if examination.anesthesia.start_anesthesia_ts is not None:
        examination.anesthesia.start_anesthesia_ts = (
            examination.anesthesia.start_anesthesia_ts.replace(tzinfo=pytz.UTC)
        )
    if examination.anesthesia.stop_anesthesia_ts is not None:
        examination.anesthesia.stop_anesthesia_ts = (
            examination.anesthesia.stop_anesthesia_ts.replace(tzinfo=pytz.UTC)
        )
...
def get_date_frequency_from_count(count):
    ...
    return (
        [
            {"date": key.strftime("%d.%m.%Y"), "count": value}
    ...
    try:
        ...
        for _, ex in df.iterrows():
            if ex.Terminvorgaben.strip() in [
                ...
                res = await upsert_md_intervention(
                    {"name": ex.Mandant}, {"name": ex.Mandant}
                )
...
class MKDateTest(BaseModel):
    ...
    class Config:
        json_encoders = {dt.datetime: lambda v: v.isoformat()[:-3] + "Z"}
...
async def set_date(mk_datetest: MKDateTest, response: Response, request: Request):
    ...
    result = await db.datetest.replace_one(
...
async def set_dates(response: Response, request: Request):
    d1 = {"id": "101", "ts": dt.datetime.utcnow(), "comment": "utcnow"}
    _ = await db.datetest.replace_one({"id": d1["id"]}, d1, upsert=True)
    d2 = {"id": "102", "ts": dt.datetime.now(), "comment": "now()"}
...
def sleep(delta: int, response: Response, request: Request):
    ...
    return {
        "delta": delta,
        "message": {"code": "code1", "text": f"{delta} Sekunden geschlafen"},
        "utc_time": t1,
    }
